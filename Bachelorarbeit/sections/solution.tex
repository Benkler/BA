...
Such dependencies are: i) data objects being accessed by the same task ii) a data object value that is used when writing to (or creating) another data object. \\
...
Therefore, two vertices that represent a pair of data objects which are accessed by the same task is to be connected by an edge. \\
...
With this in mind, the second option seems to be the most appropriate one to determine the data flow dependencies based on the data flow graph. Still, the number \textit{n} has to be defined. Small numbers illustrate represent the case when data flows is accessed 

Addiieren


activitäten nachbarn sowieso gleich ==> 0
