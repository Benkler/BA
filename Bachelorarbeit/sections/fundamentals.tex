...
Extracting the Data Flow works similar to the previous process. First of all, control flow related parts like sequence flows arcs, gateways, events and triggers are deleted. The remaining parts are tasks, data objects and data associations. Consequently, some of the tasks are not connected to the rest which results in an incoherent graph. To re-establish the connection, follow the previously deleted control flow and reconnect the disconnected tasks with data association arcs by applying the following rules:
